...
if [ "x$1" = "xdaemon" ]; then
  shift
  nohup ${JAVA} ${KS_ROOT_DIR} ${KS_LOG4J_OPTS} -cp ${CLASSPATH} com.unimas.kstream.JettyServer "start" > "$log_dir/ks.out" 2>&1 < /dev/null &
else
  exec ${JAVA} ${KS_ROOT_DIR} ${KS_LOG4J_OPTS} -cp ${CLASSPATH} com.unimas.kstream.JettyServer "$@"
fi
