...
import com.ks.bean.ExportInputStream;
 ...
 */
public class TaskQuote extends Thread {
    ...
    public TaskQuote(String app_id) {
        this.appId = app_id;
    }
    ...
    public void run() {
        ...
        try {
            ...
            if (exitCode == 0) {
                JettyServer.caches.get(appId).setStatus(AppInfo.Status.START);
            } else {
                logger.warn(appId + " process code " + exitCode);
                JettyServer.caches.get(appId).setStatus(AppInfo.Status.ODD);
            }
        } catch (IOException e) {
